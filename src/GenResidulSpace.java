...
public class GenResidulSpace {
    public Container[] genResidulSpace(Container space, Block block) {
        ...
        spaceZ.setCoordinatePoint(space.x, space.y, block.lz);
        c[0] = spaceZ;

        if (my >= mx) {
            Container spaceY = new Container().setContainer(space.lx, my, space.lz);
            spaceY.setCoordinatePoint(space.x, block.ly, space.z);
            Container spaceX = new Container().setContainer(mx, block.ly, space.lz);
            spaceX.setCoordinatePoint(block.lx, space.y, space.z);
            c[0].cutway = "Y";
            c[1] = spaceX;
            c[1].cutway = "Y";
            c[2] = spaceY;
            ...
            spaceX.setCoordinatePoint(space.x, block.ly, space.z);
            Container spaceY = new Container().setContainer(block.lx, my, space.lz);
            spaceY.setCoordinatePoint(block.lx, space.y, space.z);
            c[0].cutway = "X";
            c[1] = spaceY;
            c[1].cutway = "X";
            c[2] = spaceX;
